---
title: Customizing Gibbon
categories:
  - support
  - customization
tags: 
order: 10
contributors:
  - rossdotparker
  - jcarney2024
---
# Customizing Gibbon

Gibbon is designed on the idea that every school is different, and hopes to provide tools to meet a wide range of needs. The approach to customizing Gibbon to your school needs depends on the scope of change you're looking to make.

1. [String Replacement](/tutorials/customizing-gibbon/string-replacement)
2. [Custom Fields](/tutorials/customizing-gibbon/custom-fields)
3. [Display Settings](/tutorials/customizing-gibbon/display-settings)
4. [User Permissions](/tutorials/school-setup/roles-permissions)
5. [Modules](/guides/administration/modules)
6. Themes
7. [Feature Requests](https://ask.gibbonedu.org/c/feature-requests)
